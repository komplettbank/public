...
function AzureBlobUtil() {
    ...
    /*
        ...
        The values are returned in an object, defined like this:
        ...
        Parameters:
            ...
            accountKey      = The base64-encoded Azure Storage Account key key used to generate the signature (SAS). Required.
            containerName   = Name of container to access. Required.
            argOptions      = Optional object to override any of the default options. For example, to extend token validity to an hour, 
                              and reduce the permissions to read-only, and turn on verbose logging, you could pass the following: 
                              { "expiryPeriod" : 3600, "permissions": "r", "verboseLogging": true }
    ...
    */
    _module.getAuthDetails = (accountName, accountKey, containerName, argOptions) => {
        ...
        if (!accountKey) {
            console.error("Missing value for required parameter 'accountKey'");
            return null;
        }

        if (!containerName) {
            console.error("Missing value for required parameter 'containerName'");
            return null;
        }

        // These are the default options used to generate the querystring parameters
        let defaultOptions = {
            expiryPeriod: 60,
            permissions: 'rwd',
            serviceType: 'blob',
        ...
        // Merge options passed as argument to the function with the default options (only the first object is mutated and returned, later properties overwrite earlier properties with the same name).
        let options = Object.assign({}, defaultOptions, argOptions);

        // Whether or not to log verbosely
        _logVerbose = options.verboseLogging;

        _log("Using the following options to create the querystring params:");
        _log(options);

        let sas = new _AzureStorageSas(accountName, accountKey);
        let params = sas.getSignedQueryStringParams(options);

        _log("Generated querystring: " + sas.getSignedQueryStringFromParams(params));

        sas.urlEncodeAllPropertyValues(params);
        _log(params);
    ...
    /* 
    /* Ref: https://blog.jongallant.com/2017/03/azure-storage-sas-tokens-postman/
    /* Code initially taken from https://github.com/jongio/azure-sas-tokens-postman, 
    /* but slightly modified: 
    /*  - ESLint errors fixed.
    /*  - Modified to return querystring parameters as an object instead of a string
    /*  - Added _ to properties and functions, to clearly mark them as private
    /*  - Added the function urlEncodeAllPropertyValues()
    /*  - Minor refactoring
    /*
    /* ======================================================= */

    function _AzureStorageSas(storageAccount, storageAccessKey, usePathStyleUri) {
        this.storageAccount = storageAccount;
        this.storageAccessKey = storageAccessKey;
        this.usePathStyleUri = usePathStyleUri;
    }

    _AzureStorageSas.prototype.generateAccountSignedQueryString = function (sharedAccessPolicy) {
        ...
        addIfNotNull(queryString, _QueryStringConstants.SIGNED_VERSION, _HeaderConstants.TARGET_STORAGE_VERSION);

        // add shared access policy params
        if (sharedAccessPolicy.AccessPolicy) {
            formatAccessPolicyDates(sharedAccessPolicy.AccessPolicy);

            addIfNotNull(queryString, _QueryStringConstants.SIGNED_SERVICES, sharedAccessPolicy.AccessPolicy.Services);
            addIfNotNull(queryString, _QueryStringConstants.SIGNED_RESOURCE_TYPES, sharedAccessPolicy.AccessPolicy.ResourceTypes);
            addIfNotNull(queryString, _QueryStringConstants.SIGNED_PERMISSIONS, sharedAccessPolicy.AccessPolicy.Permissions);
            addIfNotNull(queryString, _QueryStringConstants.SIGNED_START, sharedAccessPolicy.AccessPolicy.Start);
            addIfNotNull(queryString, _QueryStringConstants.SIGNED_EXPIRY, sharedAccessPolicy.AccessPolicy.Expiry);
            addIfNotNull(queryString, _QueryStringConstants.SIGNED_IP, sharedAccessPolicy.AccessPolicy.IPAddressOrRange);
            addIfNotNull(queryString, _QueryStringConstants.SIGNED_PROTOCOL, sharedAccessPolicy.AccessPolicy.Protocols);
        }

        // add signature
        addIfNotNull(queryString, _QueryStringConstants.SIGNATURE, this._generateAccountSharedAccessSignature(sharedAccessPolicy));

        return this.queryStringStringify(queryString);
    };

    _AzureStorageSas.prototype._generateAccountSharedAccessSignature = function (sharedAccessPolicy) {
        ...
        var stringToSign = getvalueToAppend(this.storageAccount) +
            ...
            getvalueToAppend(_HeaderConstants.TARGET_STORAGE_VERSION);

        return this.signer.sign(stringToSign);
    };

    _AzureStorageSas.prototype.signRequest = function (webResource, callback) {
        var getvalueToAppend = function (value, headerName) {
            // Do not sign content-length 0 in 2014-08-16 and later
            if (headerName === _HeaderConstants.CONTENT_LENGTH && (objectIsNull(value[headerName]) || value[headerName].toString() === '0')) {
                return '\n';
        ...
        var stringToSign =
            webResource.method + '\n' +
            getvalueToAppend(webResource.headers, _HeaderConstants.CONTENT_ENCODING) +
            getvalueToAppend(webResource.headers, _HeaderConstants.CONTENT_LANGUAGE) +
            getvalueToAppend(webResource.headers, _HeaderConstants.CONTENT_LENGTH) +
            getvalueToAppend(webResource.headers, _HeaderConstants.CONTENT_MD5) +
            getvalueToAppend(webResource.headers, _HeaderConstants.CONTENT_TYPE) +
            getvalueToAppend(webResource.headers, _HeaderConstants.DATE) +
            getvalueToAppend(webResource.headers, _HeaderConstants.IF_MODIFIED_SINCE) +
            getvalueToAppend(webResource.headers, _HeaderConstants.IF_MATCH) +
            getvalueToAppend(webResource.headers, _HeaderConstants.IF_NONE_MATCH) +
            getvalueToAppend(webResource.headers, _HeaderConstants.IF_UNMODIFIED_SINCE) +
            getvalueToAppend(webResource.headers, _HeaderConstants.RANGE) +
        ...
        webResource.withHeader(_HeaderConstants.AUTHORIZATION, 'AzureStorageSas ' + this.storageAccount + ':' + signature);
        callback(null);
    };

    _AzureStorageSas.prototype._getCanonicalizedResource = function (webResource) {
        var path = '/';
        if (webResource.path) {
    ...
    _AzureStorageSas.prototype._getCanonicalizedHeaders = function (webResource) {
        // Build canonicalized headers
        var canonicalizedHeaders = '';
        if (webResource.headers) {
            var canonicalizedHeadersArray = [];
            for (var header in webResource.headers) {
                if (header.indexOf(_HeaderConstants.PREFIX_FOR_STORAGE) === 0) {
    ...
    _AzureStorageSas.prototype.getSignedQueryStringParams = function (options) {
        ...
        return this.generateSignedQueryStringParams(options.serviceType, resourceName, sharedAccessPolicy, options.sasVersion, { resourceType: options.resourceType });
    };

    _AzureStorageSas.prototype.getSignedQueryString = function (options) {
        var params = this.getSignedQueryStringParams(options);
        return this.queryStringStringify(params);
    };

    _AzureStorageSas.prototype.getSignedQueryStringFromParams = function (params) {
        return this.queryStringStringify(params);
    };

    _AzureStorageSas.prototype.createResourceName = function (containerName, blobName, forSAS) {
        // Resource name
        if (blobName && !forSAS) {
    ...
    _AzureStorageSas.prototype.generateSignedQueryStringParams = function (serviceType, path, sharedAccessPolicy, sasVersion, args) {
        var addIfNotNull = function (queryString, name, value) {
        ...
        if (sharedAccessPolicy.AccessPolicy) {
            ...
            // tables only
            addIfNotNull(queryString, _QueryStringConstants.STARTPK, sharedAccessPolicy.AccessPolicy.StartPk);
            addIfNotNull(queryString, _QueryStringConstants.ENDPK, sharedAccessPolicy.AccessPolicy.EndPk);
            addIfNotNull(queryString, _QueryStringConstants.STARTRK, sharedAccessPolicy.AccessPolicy.StartRk);
            addIfNotNull(queryString, _QueryStringConstants.ENDRK, sharedAccessPolicy.AccessPolicy.EndRk);
        }

        // validate and add version
        var validatedSASVersionString = sasVersion; //validateVersion(sasVersion);
        addIfNotNull(queryString, _QueryStringConstants.SIGNED_VERSION, validatedSASVersionString);

        // add signed identifier
        addIfNotNull(queryString, _QueryStringConstants.SIGNED_IDENTIFIER, sharedAccessPolicy.Id);

        // blobs only
        addIfNotNull(queryString, _QueryStringConstants.SIGNED_RESOURCE, resourceType);
        if (headers) {
            addIfNotNull(queryString, _QueryStringConstants.CACHE_CONTROL, headers.cacheControl);
            addIfNotNull(queryString, _QueryStringConstants.CONTENT_TYPE, headers.contentType);
            addIfNotNull(queryString, _QueryStringConstants.CONTENT_ENCODING, headers.contentEncoding);
            addIfNotNull(queryString, _QueryStringConstants.CONTENT_LANGUAGE, headers.contentLanguage);
            addIfNotNull(queryString, _QueryStringConstants.CONTENT_DISPOSITION, headers.contentDisposition);
        }

        // tables only
        addIfNotNull(queryString, _QueryStringConstants.TABLENAME, tableName);

        // add signature
        addIfNotNull(queryString, _QueryStringConstants.SIGNATURE, this._generateSignature(serviceType, path, sharedAccessPolicy, validatedSASVersionString, { resourceType: resourceType, headers: headers, tableName: tableName }));

        //return this.queryStringStringify(queryString);
        return queryString;
    };

    _AzureStorageSas.prototype._generateSignature = function (serviceType, path, sharedAccessPolicy, sasVersion, args) {
        var getvalueToAppend = function (value, noNewLine) {
            var returnValue = '';
    ...
    _AzureStorageSas.prototype.signString = function (stringToSign) {
    ...
    _AzureStorageSas.prototype.objectIsNull = function (value) {
        return _.isNull(value) || _.isUndefined(value);
    };

    _AzureStorageSas.prototype.truncatedISO8061Date = function (date) {
        var dateString = date.toISOString();
        return dateString.substring(0, dateString.length - 5) + 'Z';
    };

    //AzureStorageSas.prototype.queryStringStringify = 
    let _stringifyPrimitive = function (v) {
        switch (typeof v) {
            case 'string':
                return v;
    ...
    _AzureStorageSas.prototype.queryStringStringify = function (obj, sep, eq, name) {
        sep = sep || '&';
        eq = eq || '=';
        if (obj === null) {
            obj = undefined;
    ...
    _AzureStorageSas.prototype.urlEncodeAllPropertyValues = function (obj) {
        if (obj === null) {
            obj = undefined;
        }

        if (typeof obj === 'object') {
            Object.keys(obj).forEach(k => obj[k] = encodeURIComponent(_stringifyPrimitive(obj[k])));
        }
    };

    let _storageDnsSuffix = 'core.windows.net'; //process.env.AZURE_STORAGE_DNS_SUFFIX || 'core.windows.net';
    ...
    let _Constants = {
        ...
        */
        ...
        StorageServiceClientConstants: {
            ...
            */
            CLOUD_BLOB_HOST: 'blob.' + _storageDnsSuffix,
            CLOUD_QUEUE_HOST: 'queue.' + _storageDnsSuffix,
            CLOUD_TABLE_HOST: 'table.' + _storageDnsSuffix,
            CLOUD_FILE_HOST: 'file.' + _storageDnsSuffix
        },
